...
lableSwichingByCluster = function(X,taoList,Ylist, Mlist, psyList,lambdaList,m,Zlist){
  ...
  for(i in 1:nsim){
    ...
    for(j in 1:m){
      ...
      tempVar = varList[[j]]
      funList = function(x){
        dmvnorm(x = x, mean = tempMean, sigma =  tempVar)
      }
      distValue[,j] = apply(X = t(X), MARGIN = 1, FUN = funList)
    }
  ...
  return(list(ZswitchList=ZswitchList, taoSwitchList = taoSwitchList, MswitchList = MswitchList,
              YswitchList = YswitchList, lambdaSwitchList = lambdaSwitchList, psySwitchList = psySwitchList))
}
