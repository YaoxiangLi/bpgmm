...
This vignette uses a different simulation from the model-selection article.
Here the point is not to recover \(m\); it is to show how posterior output can
be post-processed to rank variables. The first three variables have different
cluster means, variables four and five have mostly covariance differences, and
variable six is weak noise.
...
This is not a Bayesian variable-selection posterior. It is a compact way to
ask which observed variables separate the fitted clusters.
...
model has been fitted. A careful distinction makes the package easier to use
correctly and avoids overstating the statistical target.
