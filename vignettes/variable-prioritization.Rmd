...
For cluster \(k\), the loading matrix \(\Lambda_k \in \mathbb{R}^{p \times q_k}\)
enters the covariance through \(\Lambda_k \Lambda_k^\top\). A simple exploratory
summary is the posterior mean absolute loading
...
which ranks variables by average factor loading magnitude in cluster \(k\).
