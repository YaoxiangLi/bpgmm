...
models. It is used for model-based clustering when the number of
...
The example below creates two small clusters, fits a short RJMCMC chain,
and summarizes the posterior samples. Applied analyses should use a
longer burn-in and more posterior samples.
...
In this call, `X` is a numeric matrix with variables in rows and
observations in columns. Set `m_step = 1` to allow RJMCMC updates for
the number of clusters and `v_step = 1` to allow updates for the
variance structure.
...
Individual RJMCMC iterations are sequential because each state depends
on the previous state. To use multiple cores safely, run independent
chains in parallel:
...
This uses separate worker processes where available and stores
deterministic per-chain seeds in `attr(fits, "chain_seeds")`.
